   ...
   const products = productsArr[1].posts
    ...
    return(
       ...
       <div className={styles.post_page_container} >
        ...
        {posts.map(({url,userIndex,},index) => <div className={styles.posts_container}><img  className={styles.posts_img} src ={url}/>{products[index].comments.map(el =><h3>{el}</h3>)}<span className={styles.posts_span}>Likes:{products[index].likes}</span><svg   onClick ={() =>{
